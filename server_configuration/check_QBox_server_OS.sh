...
linuxType=$( cat /etc/lsb-release | grep DISTRIB_ID)
linuxType=${linuxType#*=}

#if "$count_ubuntu" -ge 0 
#    then linuxType="ubuntu"
#else if "$count_Ubuntu" -ge 0 
#    then linuxType="Ubuntu"
#else
...
ubuntuVersion=$( cat /etc/lsb-release | grep DISTRIB_RELEASE )
